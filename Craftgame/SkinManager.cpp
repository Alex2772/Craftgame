...
size_t* SkinManager::getSkin(std::string name) {
	static size_t ds = CGE::instance->textureManager->loadTexture(_R("craftgame:res/entity/default.png"));
	...
	skins[name] = skin;
	CGE::instance->threadPool.runAsync([name](size_t* pointer) {
		try {
			std::stringstream ss;
			http_get(std::string("http://craftgame.alex2772.ru/Craftgame/CraftgameSkins/") + name + ".png", &ss);
